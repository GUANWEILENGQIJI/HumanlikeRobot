...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.stand_flag = torch.randint(0, 2, (self.num_envs, 1), device=self.device) * 2 - 1
        self.delta_phase = torch.randint(0, 2, (self.num_envs,), device=self.device).float() * 0.5
    ...
    def step(self, actions):
        ...
        critic_obs = self.privileged_obs_buf.clone()
        actor_obs = self.obs_buf.clone()
        reward_buf = self.rew_buf.clone()
    ...
    def update_obs_latency_buffer(self):
        ...
        if self.cfg.domain_rand.randomize_obs_imu_latency:
            ...
            self.obs_imu_latency_buffer[:,:,0] = torch.cat((self.base_ang_vel * self.obs_scales.ang_vel, self.base_euler_xyz), 1).clone()
    ...
    def reset_idx(self, env_ids):
        ...
        # self.delta_phase[env_ids] = torch.randint(0, 2, (len(env_ids),), device=self.device).float() * 0.5
        # reset buffers
    ...
    def _compute_torques(self, actions_scaled):
        ...
        self.raw_torques = torques.clone()
