...
class Logger:
    ...
    def _plot_test(self):
        ...
        a = axs[0]
        if log["dof_pos"]: a.plot(time, log["dof_pos"], label='measured')
        if log["dof_ref_pos"]: a.plot(time, log["dof_ref_pos"], label='reference')
        if log["mujoco_pos"]: a.plot(time, log["mujoco_pos"], label='mujoco')
        a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position')
        a.legend()
        # plot joint velocity
        a = axs[1]
        if log["dof_vel"]: a.plot(time, log["dof_vel"], label='measured')
        if log["mujoco_vel"]: a.plot(time, log["mujoco_vel"], label='mujoco')
        a.set(xlabel='time [s]', ylabel='Velocity [rad/s]', title='Joint Velocity')
        a.legend()
        plt.show()
